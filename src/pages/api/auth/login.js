...
export default async function (req, res) {
  ...
  const { username, password, accountNo, schoolNo } = await req.body
  ...
  // const accountLookup = await Account.find({ id: account });
  // if (accountLookup.length <= 0) {
  //   res.status(401).json({
  //     message: "Invalid account ID",
  //   });
  // } else {
  //   const userList = await User.find({ account: account });
